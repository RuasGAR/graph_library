...
def distancia(
    grafo: Union[MatrizAdj, VetorAdj], origem: int, destino: int
) -> Union[int, float]:
...
def componentes_conexas(grafo: Union[MatrizAdj, VetorAdj]):
    ...
    desmarcados: List[Vertice] = list(
        map(lambda v: v.valor, deepcopy(busca_em_grafo.vertices))
    )
    ...
    while len(desmarcados) > 0:  # Checagem 0(1)
        ...
        for vertice in componente:  # c vertices
            if vertice.valor in desmarcados:  # n VAI CAINDO
                try:
                    del desmarcados[desmarcados.index(vertice.valor)]
                except:
                    continue
